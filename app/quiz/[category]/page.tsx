...
export default function QuizSolverPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const handleSubmit = async () => {
    ...
    const isCorrect = selectedOption === currentQuestion.answer_id
    await saveAttempt(currentQuestion.id, selectedOption, isCorrect)
    
    if (isCorrect) setEarnedXp(10)
  }
  ...
  const isCorrect = isSubmitted && selectedOption === currentQuestion.answer_id
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-3">
            {currentQuestion.options.map((option) => {
              ...
              const isAnswer = currentQuestion.answer_id === option.id
          ...
          {isSubmitted ? (
            <div className={`p-4 rounded-xl border ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className="flex items-center justify-between mb-2">
                ...
                {isCorrect && earnedXp && (
                  <span className="px-2 py-1 bg-green-200 text-green-800 text-xs font-black rounded-lg animate-bounce">XP 획득!</span>
                )}
              </div>
