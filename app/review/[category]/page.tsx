      ...
      <div className="w-full max-w-2xl space-y-6">
        ...
        <header className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">📝 나의 오답 노트</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">내가 틀렸던 문제들을 다시 복습해 보세요.</p>
          </div>
          <Link href="/quiz" className="px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700">
            대시보드로
          </Link>
        </header>
        ...
        {incorrectQuestions.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 p-12 text-center rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <h2 className="text-lg font-bold text-slate-700 dark:text-slate-200 mt-4">오답 노트가 텅 비어있어요!</h2>
            <p className="text-slate-500 dark:text-slate-400 mt-2">해당 분야에서 틀린 문제가 없습니다.</p>
          </div>
        ) : (
              ...
              return (
                <div key={q.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">상세 해설</p>
